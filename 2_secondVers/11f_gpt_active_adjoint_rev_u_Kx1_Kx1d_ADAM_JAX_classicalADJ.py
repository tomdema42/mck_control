...
Adjoint (classical, continuous-time):
...
- Adjoint: RK4 backward in time using node-wise ∂L/∂x.
- Optax ADAM is used, but gradients are manual (no jax.grad).
- This version stores the ADAM path and can plot the 2D cost landscape J(k0,k1).
...
# =============================================================================
# Main routine: simulate + classical adjoint gradient + ADAM loop
# =============================================================================
def simulate_2dof_with_optax_adam_classical_adjoint_x1x1d_control(
    ...
    c1, c2,
    kc, cd,
    F_ext,          # python callable, used ONLY to precompute forcing on the grid
    t_end,
    y0,
    N,
    w_x1, w_x1d, w_e, w_ed,
    ...
    # -------------------------------------------------------------------------
    # 1) Time grid + forcing precompute
    # -------------------------------------------------------------------------
    # We precompute forcing values in NumPy because F_ext is a Python callable
    # (not JAX-traceable). Then we convert to JAX arrays for jitted code.
    ...
    # -------------------------------------------------------------------------
    # 2) Constant system matrices A, B and forcing vector b(t)
    # -------------------------------------------------------------------------
    # State x = [x1, x1d, x2, x2d]
    # Dynamics: xdot = A x + B u + b(F_ext(t))
    A = jnp.array([
        [0.0, 1.0, 0.0, 0.0],
    ...
    # Here B is a vector (shape (4,)) because u is scalar and acts only on x2dd.
    B = jnp.array([0.0, 0.0, 0.0, 1.0 / m2])

    def forcing_vec(F_scalar):
        # External forcing enters x1dd equation as +F/m1
        return jnp.array([0.0, F_scalar / m1, 0.0, 0.0])

    # -------------------------------------------------------------------------
    # 3) Control law + cost L(x,u) and its gradient ∂L/∂x
    # -------------------------------------------------------------------------
    def k_full(k):
    ...
    def control_u(x, k):
        # u = k0*x1 + k1*x1d  (only first two states are used)
    ...
    def grad_running_cost_x(x, k):
        """
        ∂L/∂x at a node (shape (4,)).

        L = w_x1 x1^2 + w_x1d x1d^2 + w_e (x1+x2)^2 + w_ed (x1d+x2d)^2 + r_u u^2
        u = k0*x1 + k1*x1d

        The u^2 term contributes:
          ∂(r_u u^2)/∂x = 2*r_u*u * ∂u/∂x = 2*r_u*u * [k0,k1,0,0]
        """
        x1_, x1d_, x2_, x2d_ = x
        e = x1_ + x2_
        ed = x1d_ + x2d_
        ...
        # First the parts independent of u
        g = jnp.array([
            2.0 * (w_x1 * x1_ + w_e * e),
        ...
        # Add control penalty contribution (only x1, x1d)
        g = g + 2.0 * r_u * u * k_full(k)
        return g

    # -------------------------------------------------------------------------
    # 4) Closed-loop dynamics f(x,k,t) and RK4 integrator (forward)
    # -------------------------------------------------------------------------
    def A_cl(k):
        """
        Closed-loop Jacobian:
            ∂f/∂x = A + B*K_full
        For scalar u = K_full x, the product B*K_full is an outer product.
        """
    ...
    def rk4_step_state(x, k, Fi, Fh, Fi1):
        """
        One RK4 step from node i to i+1.
        We use Fi = F(t_i), Fh = F(t_i + dt/2), Fi1 = F(t_{i+1}).
        """
        k1_ = f_closed(x, k, Fi)
    ...
    def forward_traj(k):
        """
        Simulate forward and return state at all nodes:
            X shape = (N+1, 4), with X[0]=y0 and X[i] = state at t[i].

        Implementation detail:
        - lax.scan runs a loop inside JAX efficiently.
        - It returns (carry_final, stacked_outputs).
        Here:
          carry = current state x
          output = next state x_next (so we collect states on nodes 1..N)
        """
        def step(x, i):
            Fi = F_nodes[i]
            Fh = F_half[i]
            ...
            return x_next, x_next  # (new_carry, output_to_stack)

        # X_next has shape (N, 4): states at nodes 1..N
        _, X_next = jax.lax.scan(step, y0, jnp.arange(N))

        # Prepend initial condition -> full trajectory (N+1, 4)
        X = jnp.vstack([y0[None, :], X_next])
        return X

    # -------------------------------------------------------------------------
    # 5) Cost on a trajectory (trapezoidal rule)
    # -------------------------------------------------------------------------
    def cost_from_traj(k, X):
        """
        Returns:
          J: scalar
          u: (N+1,) control values on nodes
          L: (N+1,) running cost values on nodes
        """
        u = X[:, :2] @ k                   # u_i = [x1_i, x1d_i] dot k
        L = jax.vmap(running_cost)(X, u)   # vectorized node-wise L(x_i,u_i)

        # Trapezoidal rule over nodes
        J = jnp.sum(0.5 * dt * (L[:-1] + L[1:]))
        return J, u, L

    # -------------------------------------------------------------------------
    # 6) Classical continuous adjoint (integrate backward)
    # -------------------------------------------------------------------------
    def adjoint_from_traj(k, X):
        ...
        A simple way to implement a backward integration on a forward time grid:
        - Reverse time: τ = T - t
          Then:
            dλ/dτ = +Acl^T λ + ∂L/∂x
        - Integrate forward in τ from 0 to T (which corresponds to t from T to 0)
        - Finally reverse λ back to match the original t-order.
        """
        AclT = A_cl(k).T  # constant for this restricted linear feedback (k constant)

        # gradL[i] = ∂L/∂x evaluated at node i
        gradL = jax.vmap(lambda x: grad_running_cost_x(x, k))(X)  # (N+1,4)

        # Work in reversed order (node N -> node 0)
        gradL_rev = gradL[::-1]

        def rhs_rev(lam, gradL_here):
            # dλ/dτ = Acl^T λ + ∂L/∂x
            return AclT @ lam + gradL_here

        def rk4_step_adj_rev(lam, g_curr, g_mid, g_next):
            """
            RK4 step for λ in reversed-time variable τ.
            Using gradL at current and next reversed nodes; mid-point uses average.
            """
        ...
        lam0 = jnp.zeros((4,))  # lambda(T)=0 corresponds to τ=0

        def step(lam, j):
            # We march forward in reversed indexing: j=0..N-1
            g_curr = gradL_rev[j]
        ...
        # lam_next contains λ at reversed nodes 1..N in τ
        _, lam_next = jax.lax.scan(step, lam0, jnp.arange(N))
        lam_rev = jnp.vstack([lam0[None, :], lam_next])  # reversed order

        # Flip back so lam[i] corresponds to time t[i]
        lam = lam_rev[::-1]
        return lam

    # -------------------------------------------------------------------------
    # 7) Gradient wrt k (2 parameters)
    # -------------------------------------------------------------------------
    def grad_wrt_k(k, X, u, lam):
        ...
        Explanation of the scalar term:
          - 2*r_u*u comes from ∂(r_u u^2)/∂u = 2*r_u*u and chain ∂u/∂k = [x1,x1d]
          - B^T*lambda appears from the Hamiltonian term λ^T f and ∂f/∂u = B
        """
        BTlam = lam @ B               # (N+1,) since B is (4,)
        s = 2.0 * r_u * u + BTlam     # (N+1,)
        g_nodes = X[:, :2] * s[:, None]  # (N+1,2)

        # Trapezoidal rule over time
        gk = jnp.sum(0.5 * dt * (g_nodes[:-1] + g_nodes[1:]), axis=0)  # (2,)
        return gk

    # -------------------------------------------------------------------------
    # 8) JIT-ed wrappers (one forward + one backward per gradient call)
    # -------------------------------------------------------------------------
    @jax.jit
    ...
    # Choose best iterate (tracking best is often nicer than last for ADAM)
    if best_k is None:
        k_opt = np.array(k)
    ...
    # -------------------------------------------------------------------------
    # 10) Produce passive and optimal trajectories
    # -------------------------------------------------------------------------
    # Passive = u=0 => k=[0,0]
    ...
    info = {
        ...
        # Expose a callable for cost landscape evaluation
        "cost_only": cost_only,
        # Extra (sometimes handy when debugging)
