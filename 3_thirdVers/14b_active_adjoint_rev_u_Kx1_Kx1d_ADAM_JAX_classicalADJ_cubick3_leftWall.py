...
NOW WITH A DUFFING (CUBIC) NONLINEARITY ON DOF1 (LEFT WALL / MASS m1):
...
Closed-loop nonlinear dynamics:
    xdot = A x + B u + b(t) + g_nl(x)
with Duffing term affecting only x1dd:
    x1dd += -(k3/m1) * x1^3
...
Key change vs linear case:
- ∂f/∂x is NOT constant anymore because of Duffing term:
    ∂/∂x1 [ -(k3/m1) x1^3 ] = -(3*k3/m1) x1^2
  so the Jacobian gets an extra contribution in (row x1dd, col x1).

Gradient wrt k (still same structure, because Duffing term does NOT depend on u):
...
- Adjoint: RK4 backward in time (implemented via time reversal),
  using the state-dependent Jacobian ∂f/∂x evaluated along the trajectory.
...
def precompute_time_and_forcing(F_ext, t_end, N):
    ...
    F_ext is a Python callable (not JAX-traceable), so we evaluate it in NumPy,
    then convert the forcing arrays to JAX.
    """
...
def build_linear_system(m1, m2, k1, k2, c1, c2, kc, cd):
    """
    Build constant matrices for the LINEAR part of the 2-DOF mass-spring-damper system.
...
# =============================================================================
# Core solver + adjoint + optimization (Duffing nonlinear plant on DOF1)
# =============================================================================
def simulate_2dof_with_optax_adam_classical_adjoint_x1x1d_control_duffing(
    m1,
    m2,
    k1,
    k2,
    ...
    kc,
    cd,
    k3,     # Duffing cubic stiffness coefficient (now on DOF1)
    ...
    # -------------------------------------------------------------------------
    # 2) Linear system matrices and forcing mapping
    # -------------------------------------------------------------------------
    A, B, forcing_vec = build_linear_system(m1, m2, k1, k2, c1, c2, kc, cd)

    # Duffing constants for DOF1 (x1dd equation)
    k3_over_m1 = k3 / m1
    dk3dx1_over_m1 = (3.0 * k3) / m1  # used in Jacobian: -(3*k3/m1)*x1^2
    ...
    # -------------------------------------------------------------------------
    # 4) Nonlinear closed-loop dynamics + Jacobian wrt x
    # -------------------------------------------------------------------------
    def A_cl(k):
        """
    ...
    def g_duffing(x):
        """
        Duffing cubic stiffness term on DOF1 (affects only x1dd):
            x1dd += -(k3/m1) * x1^3
        """
        return jnp.array([0.0, -k3_over_m1 * x[0] ** 3, 0.0, 0.0])

    def f_closed(x, k, F_scalar):
        """
        Nonlinear dynamics:
            xdot = A x + B u + b(F) + g_duffing(x)
        """
        u = control_u(x, k)
        return A @ x + B * u + forcing_vec(F_scalar) + g_duffing(x)

    def jac_f_x(x, k):
        """
        State Jacobian ∂f/∂x (shape 4x4), evaluated at state x:

          ∂f/∂x = (A + outer(B, K_full)) + J_duff(x)

        Duffing contribution (DOF1):
          x1dd term: -(k3/m1) x1^3
          d/dx1:     -(3*k3/m1) x1^2

        So:
          J_duff has only one nonzero entry:
            (row=1, col=0) = -(3*k3/m1) * x1^2
        """
        J = A_cl(k)
        extra = jnp.zeros((4, 4)).at[1, 0].set(-dk3dx1_over_m1 * x[0] ** 2)
        return J + extra
    ...
    # -------------------------------------------------------------------------
    # 7) Classical continuous adjoint (time reversal) with state-dependent Jacobian
    # -------------------------------------------------------------------------
    def adjoint_from_traj(k, X):
        """
        Adjoint ODE (continuous-time):
          lambda_dot = -Jx(t)^T lambda - (∂L/∂x)^T,   lambda(T)=0
        where Jx(t) = ∂f/∂x evaluated along X(t).

        Time reversal trick:
          τ = T - t  =>  dλ/dτ = +Jx(t)^T λ + ∂L/∂x
          integrate forward in τ on reversed arrays, then flip back.
        """
        gradL = jax.vmap(lambda x: grad_running_cost_x(x, k))(X)   # (N+1,4)
        Jx_nodes = jax.vmap(lambda x: jac_f_x(x, k))(X)            # (N+1,4,4)

        gradL_rev = gradL[::-1]
        Jx_rev = Jx_nodes[::-1]
        JxT_rev = jnp.swapaxes(Jx_rev, 1, 2)

        def rhs_rev(lam, JxT_here, g_here):
            return JxT_here @ lam + g_here
    ...
    def grad_wrt_k(k, X, u, lam):
        ...
        df/du = B (still constant), Duffing term does not involve u.
        """
        BTlam = lam @ B
        s = 2.0 * r_u * u + BTlam
        g_nodes = X[:, :2] * s[:, None]
    ...
    for it in range(1, max_iter + 1):
        ...
        if it % print_every == 0 or it == 1:
            print(
                f"[Duffing@DOF1 + Classical Adjoint + ADAM | u=k0*x1+k1*x1d] "
    ...
    print(np.array(k_opt))
    info = {
        "success": True,
        "message": "Duffing nonlinear plant on DOF1 + classical adjoint + Optax ADAM completed (u depends only on x1,x1d)",
        "nit": it_final,
        "J": J_final,
        "k_opt": k_opt,
...
if __name__ == "__main__":
    ...
    # Duffing coefficient (now on DOF1)
    k3 = 50
    ...
    # -----------------------------
    # Run adjoint + ADAM (Duffing plant on DOF1)
    # -----------------------------
    t, X0, X1, u_nodes, k_opt, info, K_hist = simulate_2dof_with_optax_adam_classical_adjoint_x1x1d_control_duffing(
        m1,
        m2,
        k1,
        k2,
